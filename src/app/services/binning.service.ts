...
export class BinningService {
  ...
  makeBins(display: Display): Bin[] {
    ...
    const binWidth =
      Math.round(((binning.max - binning.min) * 100) / binning.count) / 100;
    let min = Math.round(binning.min * 100) / 100;

    // Low outliers
    bins.push(
      new Bin(0, coloring.outliers[0], -1, "icon-group-0", min, data.min)
    );
